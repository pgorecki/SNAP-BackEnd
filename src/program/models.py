...
class Program(ObjectRoot):
    ...
    agency = models.ForeignKey(
        Agency, null=True, related_name="programs", on_delete=models.SET_NULL
    )
...
class Enrollment(ObjectRoot):
    ...
    status = models.CharField(
        max_length=32, choices=[(x.name, x.value) for x in EnrollmentStatus]
    )
...
class EnrollmentActivity(ObjectRoot):
    ...
    billable_activity = models.CharField(
        max_length=6,
        blank=True,
        null=True,
    ...
    data_import_id = models.CharField(
        max_length=80, blank=True, null=True, help_text="MPR import job run instance"
    )  # MPR
...
class AttendanceWeek(models.Model):  # MPR
    ...
    activity = models.ForeignKey(
        EnrollmentActivity, on_delete=models.CASCADE, related_name="attendance_weeks"
    )  # MPR
...
class EnrollmentServiceType(models.Model):
    ...
    category = models.CharField(
        max_length=32, choices=ServiceType.choices, default=ServiceType.DIRECT
    )
...
class EnrollmentService(ObjectRoot):
    ...
    enrollment = models.ForeignKey(
        Enrollment, on_delete=models.CASCADE, related_name="services"
    )
    ...
    type_and_amount = models.CharField(
        max_length=64,
        blank=True,
        null=True,
